...
export function MethodologySection({ stepsRef, methodVisible }: MethodologySectionProps) {
  return (
    <section className="method" id="methodologie" data-bg="dark" data-screen-label="05 Souveraineté & Conformité">
      <div className="method__head">
        <h2 className="method__title">{methodologyCopy.title}</h2>
        <KereliaRule center />
        <p className="method__intro">{methodologyCopy.intro}</p>
      </div>
...
export function MetricsSection() {
  return (
    <section className="metrics" id="chiffres" data-bg="dark" data-screen-label="07 Chiffres & Témoignages">
      <div className="metrics__head">
        <h2 className="metrics__title">{metricsCopy.title}</h2>
        <KereliaRule center />
      </div>
      <div className="metrics__grid">
        {metricsCopy.metrics.map((m) => (
          <div key={m.value} className="metrics__item">
            <span className="metrics__value">{m.value}</span>
            <p className="metrics__label">{m.label}</p>
          </div>
        ))}
      </div>
      <div className="metrics__testimonials">
        {metricsCopy.testimonials.map((t) => (
          <blockquote key={t.name} className="metrics__quote">
            <p className="metrics__quote-text">&ldquo;{t.quote}&rdquo;</p>
            <footer className="metrics__quote-footer">
              <strong>{t.name}</strong> &mdash; {t.title}, {t.org}
            </footer>
          </blockquote>
        ))}
      </div>
...
export function AboutSection() {
  return (
    <section className="about" id="apropos" data-bg="dark" data-screen-label="06 Qui sommes-nous">
      <div className="about__grid">
        ...
        <div className="about__map-col about__map-col--small">
        ...
        <div className="about__partners-line">
