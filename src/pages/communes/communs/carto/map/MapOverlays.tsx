...
import HistoryPipelineCard, { type HistoryPipeline } from "../tools/HistoryPipelineCard";
...
/** Masque le flash de dézoom lors de l'indexation légende (tuiles communales). */
export function MapLegendHarvestOverlay({ active }: { active: boolean }) {
  if (!active) return null;

  return (
    <div
      className="absolute inset-0 z-[45] flex items-center justify-center bg-white/70 backdrop-blur-[2px] pointer-events-none"
      role="status"
      aria-live="polite"
      aria-label="Chargement de la légende"
    >
      <div className="flex flex-col items-center gap-2 rounded-lg border border-gray-200 bg-white/95 px-4 py-3 shadow-md">
        <Loader2 className="h-8 w-8 animate-spin text-slate-600" aria-hidden />
        <p className="text-xs text-gray-600">Préparation de la légende…</p>
      </div>
    </div>
  );
}
