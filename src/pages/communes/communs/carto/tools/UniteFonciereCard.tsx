...
export default function UniteFonciereCard({
  ...
  const communeCuaSlug = (communeSlug ?? dbSchema ?? "argeles").trim().toLowerCase();
  const useDirectCuaPipeline = usesDirectCuaPipeline(communeCuaSlug);
  ...
  const runGenerateCua = async () => {
    if (!useDirectCuaPipeline) return;

    const numeroCu = `CU-${parcellesCadastrales.map((p) => `${p.section}${p.numero}`).join("+")}`;
    ...
    try {
      ...
      if (result.slug) {
        setCuaSlug(result.slug);
        onPipelineCreated?.(result.slug);
      }
      if (result.docxUrl) {
        setCuaDocxUrl(result.docxUrl);
        setCuaViewerUrl(result.viewerUrl ?? buildCuaViewerPath(result.docxUrl));
      }
      if (result.carteUrl) {
