...
import { generateCommuneCua } from "../cua/generateCommuneCua";
...
export default function ParcelleCuaGenerateAction({
  ...
  const runGenerate = async () => {
    ...
    try {
      const result = await generateCommuneCua({
        communeSlug,
        refs: parcelles.map((p) => ({ section: p.section, numero: p.numero })),
        numeroCu: referenceDossier.trim(),
        ...
        <div className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-3 space-y-3">
          <div>
            <label htmlFor="cua-reference-dossier" className="block text-xs font-medium text-emerald-900 mb-1">
              Référence du dossier / demande Cerfa <span className="font-normal text-emerald-800/70">(facultatif)</span>
            </label>
            <input
            ...
            <p className="mt-1 text-[11px] text-emerald-800/80">
              Si renseignée, cette référence sera inscrite dans l&apos;en-tête du CUA et utilisée pour nommer le DOCX.
            </p>
          </div>

          <div>
            <label htmlFor="cua-demandeur-nom" className="block text-xs font-medium text-emerald-900 mb-1">
              Nom du demandeur <span className="font-normal text-emerald-800/70">(facultatif)</span>
            </label>
            <input
              id="cua-demandeur-nom"
              type="text"
              value={demandeurNom}
            ...
            />
            <p className="mt-1 text-[11px] text-emerald-800/80">
              Sera mentionné dans le CUA et conservé dans l&apos;historique des dossiers.
            </p>
          </div>
