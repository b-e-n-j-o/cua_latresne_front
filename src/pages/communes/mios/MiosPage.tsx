...
async function readCadastreFromCache(apiUrl: string): Promise<GeoJSON.FeatureCollection | null> {
  try {
    ...
    const lastSync = Number(window.localStorage.getItem(MIOS_CADASTRE_CACHE_META_KEY) || "0");
    const isFresh = Number.isFinite(lastSync) && Date.now() - lastSync < MIOS_CADASTRE_CACHE_TTL_MS;
    if (!isFresh) return null;

    const cache = await window.caches.open(MIOS_CADASTRE_CACHE_NAME);
...
async function writeCadastreToCache(apiUrl: string, data: GeoJSON.FeatureCollection): Promise<void> {
  try {
    ...
    window.localStorage.setItem(MIOS_CADASTRE_CACHE_META_KEY, String(Date.now()));
  } catch {
...
async function fetchCadastreGeojson(): Promise<GeoJSON.FeatureCollection> {
  ...
  const apiUrl = `${base}${MIOS_CADASTRE_API_PATH}`;
  ...
  if (response.url.includes(MIOS_CADASTRE_API_PATH)) {
...
export default function MiosPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("Erreur récupération session Supabase dans MiosPage", e);
      }
    })();
  }, []);
  ...
  const refreshHistoryPipelines = async (focusSlug?: string) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/pipelines/by_user?user_id=${userId}&commune_slug=mios`);
      ...
      map.on("click", "latresne_parcelles-fill", async (e) => {
        ...
        if (ufBuilderModeRef.current) {
          toggleUfParcelle({
            ...
            commune: props.commune || "Mios",
            insee: props.insee || "33284",
      ...
      map.on("click", "parcelle-search-fill", async (e) => {
        ...
        if (ufBuilderModeRef.current) {
          toggleUfParcelle({
            ...
            numero: normalizedNumero,
            commune: props.commune || "Mios",
            insee: props.insee || props.code_insee || "33284",
            geometry: feature.geometry as GeoJSON.Geometry,
            addedVia: "map",
          });
          return;
        ...
        const insee = props.code_insee ?? props.insee ?? "33284";
          ...
          onAddManualUfParcelleToMap={(section, numero, inseeInput) => {
            ...
            setSelectedUfParcelles((prev) => [
              ...prev,
              {
                ...
                numero: normalizedNumero,
                commune: props.commune || "Mios",
                insee: normalizedInsee || props.insee || "33284",
            ...
            content: (
              <UniteFonciereCard
                ...
                onPipelineCreated={(newSlug) => {
                  ...
                  navigate(`/mios/cua/projects/${newSlug}`);
                }}
    ...
    <div className="cua-map-workspace">
        <CartoLeftSidebar
          ...
          history={{
            ...
            onOpenProject: (slug) => navigate(`/mios/cua/projects/${slug}`),
